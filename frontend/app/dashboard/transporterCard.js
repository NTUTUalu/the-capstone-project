  ...
  return (
    ...
    >
      <div className="h-full w-full">
        <div
          className={` grid grid-cols-1 gap-8 w-full mt-8 justify-items-center mx-20 rounded-3xl min-w-72 max-w-xl bg-yellow-900 p-4 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.7),0_10px_20px_-2px_rgba(0,0,0,0.4)]dark:bg-neutral-7`}
        >
          <form className="w-full grid grid-cols-1 grid-rows-1 items-center justify-items-center gap-4 bg-pink-3 bg-green-3">
            <div className="grid grid-cols-2 grid-row-1 gap-4 p-3 bg-yellow-0 w-full h-48 rounded-3xl border border-white bg-pink-3 ">
              <div className="flex flex-col justify-end tracking-wide p-2 bg-pink-1 border-r-2 border-white">
                {names && (
                  <h4 className="text-amber-400 tracking-wide font-medium mb-1 text-wrap text-2xl">
                    {names}
                  </h4>
                )}
                {deliveryProvince && (
                ...
                {mobileNumber && (
                  <h4 className="text-white font- text-lg tracking-wide mb-1">
                    {mobileNumber}
                  </h4>
                )}
              </div>
              <div className="bg-blue-1 position-relative grid grid-cols-1 gap-2 pb-5 items-end bg-slate-4 h-full w-fit">
                <button
                  ...
                  className="inline-block w-fit rounded-2xl h-10 bg-amber-400 px-6 pb-2 pt-2.5 text-sm tracking-wider uppercase leading-normal text-yellow-800 font-semibold transition duration-150 ease-in-out hover:bg-amber-400"
                >
                  Copy Phone Number
                </button>
              </div>
